...
//Render::_netgui BaseNetGui = (Render::_netgui) Global::Bases::NetGui;
...
//bool console_OutputRedirect = true;
...
//Global::_gametick origGameTickFunction = NULL;
...
void PollLuaMem() {
    while (true) {
        if (debugloop) {
            __int64 consolehandle = GetConsole(consolebuffer);
            PrintLuaMem(consolehandle);
...
//    typedef int(__cdecl* GetTopHook)(lua_State*);
...
//    _debug_print DebugPrint = NULL;
//    GetTopHook GetTop = NULL;
//    _luaB_print LuaB_print = NULL;
...
//    {
//        int n = GetTop(L); /* number of arguments */
//        int i;
...
//    {
//        console_OutputRedirect = !console_OutputRedirect;
//    }
...
//                float x = offset_x;
//                float y = offset_y - font_size*itemid;
//                std::string itemname = item.GetName();
//                //spdlog::info(item.GetName().c_str());
//                if (itemid == selectedItem)
//                {
//                    AddText(GameRender, itemname.c_str(), x, y, font_size, color_picked);
//                }
//                else
//                {
//                    AddText(GameRender, itemname.c_str(), x, y, font_size, color);
//                }
//                //spdlog::info("X: {} Y:{} NAME: {} SIZE: {} COLOR: {}", x, y, itemname, font_size, color);
//                //Sleep(1000);
//                //AddText(GameRender, std::to_string(CountDown).c_str() , 0, 0, GUIConsole_charsize, 0xFF000000);
//            }
//
...
--------------------------------------------------------------------------
    ...
    std::thread thread_debugInfo(PollLuaMem);
...

    {
        ...
        while (std::getline(std::cin, input)) {
            ...
            if (command == "test") {
                ...
                int stacksize = Lua::GetTop(Lua::new_lua_State_ptr);
                std::cout << stacksize << std::endl;

                Lua::luaL_loadstring(Lua::new_lua_State_ptr,
                                     "SpawnEvent{ DisplayText = { text = \"THIS TEXT IS CALLED FROM LUA_DOFILE FROM C!\", x = 0, y = 0, duration = 3, fadeTime = 0.5, scale = 0.1 } }") ||
                Lua::lua_pcallk(Lua::new_lua_State_ptr, 0, -1, 0, 0, NULL);
                stacksize = Lua::GetTop(Lua::new_lua_State_ptr);
                std::cout << stacksize << std::endl;
                input.clear();
                commandfound = true;
            }
...
//    return Global::AddText(a1, text, x, y, size, color);
//}
...
//    CustomTextDoRender();
//    Render::AddText(Game::GameRender, "Journey Detour v1.0.1", -1.75, -1.0, 0.05, 0xFFFFFFFF);
...
//    InitializeLuaGUI();
    // newFunction = (GetTopHook)_gettop;
    // GetTopHook
...
//    }
    ...
    while (true) {
        ...
        if (GetAsyncKeyState(VK_RETURN) & 1 && Global::isNetGUIEnabled) {
